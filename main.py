...
def run_gcloud_script(deploy_id, project, region, password, instance):
    """Runs the shell script and streams output to global storage."""
    
    cmd = ['./create_cloudsql.sh', project, region, password, instance]
...
def deploy():
    ...
    thread = threading.Thread(
        ...
        args=(deploy_id, project_id, region, password, instance_id)
    )
    thread.start()
